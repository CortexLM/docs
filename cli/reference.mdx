...
description: "The supported cortex command surface: global options, run and exec, sessions, auth, agents, MCP, plugins, config, utilities and maintenance."
icon: "book"
---
...
Hotkey linking happens in Cortex Chat with `/link <ss58>`, not through a Cortex CLI pairing command. See [Pair a hotkey](/bounty/pair-a-hotkey) for the signing card and current participation prerequisites.
