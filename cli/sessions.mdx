...
```text
   ...
   > fix login redirect      2h ago · 14 messages · cortex/fix-login-redirect
     composer chip design    yesterday · 31 messages
     bump version to 0.1.7   3 days ago · 6 messages

 Enter:resume  |  f:favorite  |  d:delete  |  Esc:close
```
...
`/new` starts a new session without leaving the TUI. `/clear` empties the conversation but keeps the session.
