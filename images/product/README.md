...
| `chat-streaming.png` | A turn in flight — “Cortex is responding…” | `chat/how-it-works` |
...
| File | Surface | Used on |
| --- | --- | --- |
| `code-public.png` | `cortex.foundation/code` public page | `code/quickstart` |
...
| `bot-public.png` | `cortex.foundation/bot` public page | `bot/quickstart` |
...
Bot setup form and Computer rail, the Bot desktop app. Capture them from a
signed-in **production** account at 1280×800 and add a row above; never
substitute an SVG mock. There is no staging account to capture from — that
mirror is retired and its articles are not public.
